			...
			if newlvl > lvl:
				if server == 1186110546001535087:
					if newlvl >= 25:
						...
						await channel.send(f"<@{message.author.id}> has reached level {newlvl}! Congratulations on becoming a fan! You can now embed links, attach files, manage threads, use external sounds, and set voice channel statuses! You've also been rewarded with an exclusive chat ;)")
					elif newlvl >= 5:
